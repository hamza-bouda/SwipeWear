...
| `018_users_firebase.sql` | KAN-91 | `users.provider_uid` (TEXT) + index unique partiel `users_provider_uid_idx` ; `users.phone_number` (TEXT) | L'identité passe de Supabase à **Firebase** (email, Google, téléphone). **Firebase n'émet pas d'UUID** : son claim `sub` est une chaîne opaque de 28 caractères, alors que `user_id` est de type UUID dans dix colonnes et dans quatre modèles de `contracts/` — zone protégée (§4, §8). Plutôt que de migrer tout cela, `user_id` est dérivé par **UUIDv5** de `https://securetoken.google.com/<project>/<uid>` : déterministe (même personne, même ligne, sans table de correspondance à désynchroniser) et cloisonné par projet. La dérivation étant à sens unique, `provider_uid` conserve l'identifiant d'origine — la suppression du compte chez Firebase en a besoin. `phone_number` devient nécessaire car l'authentification par téléphone crée des comptes sans aucune adresse email. Aucune colonne UUID ni aucun contrat n'est modifié. |
| `017_users_external_auth.sql` | KAN-90 | `users.password_hash` et `users.email` deviennent NULLABLE ; ajout de `users.auth_provider` (TEXT NOT NULL DEFAULT 'password') | L'authentification passe à **Supabase** : le backend ne stocke plus aucun mot de passe, il vérifie le jeton signé par Supabase (JWKS asymétrique, ou secret HS256 partagé pour les projets non migrés) et provisionne le compte local au premier jeton valide. `password_hash` est rendu nullable plutôt que supprimé — les lignes créées avant ce changement gardent leur empreinte. `email` devient nullable car un fournisseur externe ne le garantit pas (Sign in with Apple peut le masquer) ; la contrainte UNIQUE tolère plusieurs NULL en Postgres. **`user_id` reste la clé primaire et reçoit directement le `sub` du jeton Supabase** (c'est un UUID) : un second identifiant plus une table de correspondance seraient deux façons de nommer la même personne, donc une façon pour elles de diverger. Corrige au passage une **prise de contrôle de compte** : `POST /auth/token` signait n'importe quel `user_id` fourni dans le corps, sans aucune preuve d'identité — connaître un identifiant suffisait à lire, modifier et supprimer le compte. Remplacé par `POST /auth/anonymous`, où le serveur génère l'identité. |
