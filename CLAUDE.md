...
| Backend | FastAPI + Pydantic. Frontend : Flutter / Dart (Android + iOS) |
...
| `018_notification_preferences.sql` | Mobile Flutter | Table `notification_preferences` (user_id UUID PK, preference instant|daily_digest|disabled, updated_at) pour la préférence globale du profil | Sépare la préférence globale de l'utilisateur des préférences par alerte (`alert_notification_prefs`) afin que le réglage Notifications du profil soit réellement persistant et appliqué par l'anti-spam. |
